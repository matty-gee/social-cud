...
%     all_trials_beta.nii, narrative_trials_beta.nii, decision_trials_beta.nii
%     all_trials_t.nii,    narrative_trials_t.nii,    decision_trials_t.nii
%------------------------------------------------------------------------
...
for s = 13 :numel(sub_dirs)
    ...
    try
        ...
        % Skip subject if merged outputs already exist
        out_all_beta  = fullfile(sub_out_dir, 'all_trials_beta.nii');
        out_narr_beta = fullfile(sub_out_dir, 'narrative_trials_beta.nii');
        out_dec_beta  = fullfile(sub_out_dir, 'decision_trials_beta.nii');

        out_all_t     = fullfile(sub_out_dir, 'all_trials_t.nii');
        out_narr_t    = fullfile(sub_out_dir, 'narrative_trials_t.nii');
        out_dec_t     = fullfile(sub_out_dir, 'decision_trials_t.nii');

        if isfile(out_all_beta) && isfile(out_narr_beta) && isfile(out_dec_beta) && ...
           isfile(out_all_t)    && isfile(out_narr_t)    && isfile(out_dec_t)
            fprintf('Skipping %s (merged outputs exist)\n', sub_id);
            continue
        end
        ...
        % build per-trial table (Narrative + Decision only)
        ...
        % ---------------------------------------------------------------------
        % Build LSA design: one regressor per trial
        % ---------------------------------------------------------------------
        ...
        for k = 1:nT
            ...
            trial_type = lower(string(t.trial_type)); % narrative / decision
        ...
        % ---------------------------------------------------------------------
        % Merge per-trial betas and t-maps into 4D stacks (narr/dec/all)
        % ---------------------------------------------------------------------

        is_narr = string(trials.trial_type) == "Narrative";
        is_dec  = string(trials.trial_type) == "Decision";

        beta_paths_all = cell(nT,1);
        t_paths_all    = cell(nT,1);
        for k = 1:nT
            beta_paths_all{k} = fullfile(beta_out, [tags{k} '_beta.nii']);
            t_paths_all{k}    = fullfile(t_out,    [tags{k} '_t.nii']);
        end

        merge_3d_to_4d(beta_paths_all,          out_all_beta);
        merge_3d_to_4d(beta_paths_all(is_narr), out_narr_beta);
        merge_3d_to_4d(beta_paths_all(is_dec),  out_dec_beta);

        merge_3d_to_4d(t_paths_all,             out_all_t);
        merge_3d_to_4d(t_paths_all(is_narr),    out_narr_t);
        merge_3d_to_4d(t_paths_all(is_dec),     out_dec_t);
